...
### Re-send API Callback

CoinGate provides a self-service tool in the Account Dashboard that allows developers to **re-send previously delivered API callbacks**. This is useful when your integration returned a **4xx/5xx** response, failed to process the payload, or when you want to re-test your callback handler after deploying fixes.

The tool sends the **latest real callback payload** for a specific order, refund, payout, or billing event—exactly as it was originally generated by CoinGate.

Typical use cases:

* Debugging why a callback was not handled correctly
* Re-testing integration after backend fixes
* Replaying missed callbacks without contacting CoinGate Support
