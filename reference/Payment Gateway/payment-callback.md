...
next:
  description: ''
---
Payment callback (Payment notification) will be sent to merchant's **_callback_url_** when [order status](https://developer.coingate.com/reference/order-statuses) is changed to _pending_ _confirming_, _paid_, _invalid_, _canceled_, _refunded_ or _expired_.
...
| `price_amount`     | The price set by the merchant; for example, _499.95_.                                                                                                                                                                                                                                                                                                                         |
| `price_currency`   | The currency code which defines the currency in which the merchant's goods/services are priced; for example, _USD_, _CHF_, _BTC_ (see [supported currencies](https://coingate.com/supported-currencies)).                                                                                                                                                                     |
| `receive_currency` | The currency code which defines the currency in which the merchant's settlements will be paid. Currency conversions are done by CoinGate automatically. For example: _EUR_, _USD_, _BTC_, _USDT_, etc.                                                                                                                                                                        |
...
| `pay_currency`     | The cryptocurrency in which the payment was made; for example, _BTC_, _LTC_, _ETH_.                                                                                                                                                                                                                                                                                           |
...
| `is_refundable`    | Possible values: _true_, _false_. Indicates whether or not the shopper can request a refund on the invoice. Changes in `is_refundable` will not trigger additional callbacks, but when order information is retrieved using GET or LIST, latest value will be shown.                                                                                                          |
| `created_at`       | Invoice creation time.                                                                                                                                                                                                                                                                                                                                                        |
| `token`            | Your custom token (or generated by CoinGate) to validate payment callback.                                                                                                                                                                                                                                                                                                    |
| `fees`             | Array of fees for _Paid_/_Refunded_/_Partially refunded_ order                                                                                                                                                                                                                                                                                                                |
| `paid_at`          | Time when the invoice was fully paid. Only included when the status is paid, refunded, or partially_refunded.                                                                                                                                                                                                                                                                 |
